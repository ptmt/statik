...
4. **HTML formatting applied** (minification or beautification, based on config)
5. **Final HTML is written** to output directory
...
- Preserves formatting in `<pre>`, `<code>`, `<textarea>`, `<script>`, and `<style>` tags
- Handles edge cases safely (inline event handlers, CDATA sections, etc.)
...
- **Production sites**: Use `MINIFY` for smaller file sizes and faster load times
- **Development**: Use `BEAUTIFY` or `DEFAULT` to inspect generated HTML
- **Debugging templates**: Use `BEAUTIFY` to see the structure clearly
...
1. **Safety**: jsoup properly parses HTML as a DOM tree, avoiding edge cases that break regex-based approaches
2. **Edge cases handled**: Inline event handlers (`onclick="alert('>')"`), CDATA sections, complex nesting
3. **No bugs**: Regex can't parse HTML reliably since HTML is not a regular language
4. **Already included**: jsoup is already a Statik dependency for datasource generation
5. **Battle-tested**: jsoup is a mature, widely-used library
